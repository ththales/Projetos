...
typedef struct {
    ...
    int level, xp, hp, maxHp, def, maxDef, armaAtual, itemExtra, moedas, andar;
}Personagem;
...
void informacoesJogador() {
    ...
    printf("Classe: %s\n", player.classe);
    printf("Level: %d\n", player.level);
    printf("XP: %d (%d XP para o Level %d)\n", player.xp, (int)((100*pow(1.5, player.level))-player.xp), player.level+1);
    printf("HP: %d\n", player.hp);
...
void combateInimigos() {
    int numInimigos, inimigoLevelAcima, turno = 1, dano, vivos, texto, timer, xpGanho = 0;
    ...
    while(player.hp>0 && vivos>0) {
        ...
        if(turno==1) {
            ...
            if(inimigos[opc-1].hp<=0) {
                ...
                inimigos[opc-1].hp = 0;
                vivos--;
                player.xp += inimigos[opc-1].level*10*(1+0.1*(inimigos[opc-1].level-player.level));
                xpGanho += inimigos[opc-1].level*10*(1+0.1*(inimigos[opc-1].level-player.level));
                printf("Voce ganhou %d XP nessa eliminacao!\n", inimigos[opc-1].level*10);
                if(player.xp>=(100*pow(1.5, player.level))) {
                    player.level++;
...
void salvarJogo() {
    ...
    fprintf(p, "%s\n%s\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d\n%d", player.nome, player.classe, player.level, player.xp, player.hp, player.maxHp, player.def, player.maxDef, player.armaAtual, player.itemExtra, player.andar, player.moedas);
    fclose(p);
}
