...
Arma armas[] = {
    {"Espada Longa", 200, 1}, //0
    {"Adaga Afiada", 250, 1}, //1
    {"Cajado Simples", 350, 1}, //2
    {"Machado leve", 300, 1}, //3...
    {"Lanca Fraca", 150, 1},
};
...
void combateInimigos() {
    int numInimigos, inimigoLevelAcima;
    numInimigos = (rand()%4)+1;
    Inimigo inimigos[numInimigos];
    printf("\nCOMBATE!\n");
    for(int i=0; i<numInimigos; i++) {
        inimigoLevelAcima = rand()%3;
        if(inimigoLevelAcima==0) {
